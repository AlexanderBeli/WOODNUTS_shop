...
Развернутый интернет-магазин можно посмотреть по ссылке (здесь)[https://woodnuts-shop-main.fly.dev/].
<h2>Описание:</h2>
<div>
...
## Результаты
- На 75% сокращено использование памяти в хранилище
- Разработан алгоритм поиска (Full text Search, Trigram Extension, raw query, regex) на 3 языках
- User-friendly дизайн, адаптированное под мобильные приложения
- Реализована система регистрации и аутентификации
- Настроена локализация и интернационализация
- Реализован перевод сайта на 3 языка.

<h2>Статистика:</h2>
<p>Период: с 19.03.2024 по 26.08.2024</p>
<p>Затраченное время: 45*3 = 135ч или 17 рабочих дней по 8ч (из рассчета в среднем 3ч на один этап, перечисленные ниже)</p>
<div>

<p>Что еще можно сделать: django-allauth(реализовано в другом проекте), проработать Custom Confirmation Emails(реализовано в другом проекте); автоматическое добавление email пользователя, его личных данных в поле заказа; REST API(реализовано в другом проекте); Sitemap; невозможность добавить в заказ товар, которого нет в наличии; добавить возможность указывать количество товара в наличии и соответственно ограничить количество ввода товара при заказе количеством на складе; добавить возможность увеличивать или уменьшать количество выбранных продуктов в корзине</p>
</div>
<h2>Этапы:</h2>
<div>
    <p>26.08.2024 Вручную ввел данные товаров и категорий для БД. Не нашел рабочего способа как загрузить данные в БД из сохраненного бэкапа локальной БД. Обнаружил, что не встроил логику кнопки "удалить" из корзины выбранный товар, не настроен redis и соответственно происходит сбой на заключительном этапе оформления заказа - точнее выдается ошибка, заказ переходит в список мои заказы(сформирован), но карзина не очищается; переходы внизу страницы на разделы товаров по категориям не настроены. Судя из документации fly.io не поддерживает Docker в чистом виде, соотвественно нет возможности создать и запустить docker-compose.yml и как следствие, такие сервисы как redis, nginx сразу не запустишь. Возможно DigitalOpen работает по другому и там в этом моменте проще, не проверял. Этот факт немного огорчил, учитывая что локально все это было настроено и исправно функционирует. Решил загрузить docker-compose.prod.yml для того, чтобы было понимание что и как было сделано.</p>
    <p>20.08.2024 Развернул на fly.io</p>
    <p>18.08.2024 Привязал nginx и сделал запуск через gunicorn</p>
    ...
    <p>25.06.2024 Оптимизировал и расширил функциоанл админки: теперь добавлена возможность редактировать информацию нескольких товаров одновременно.</p>
